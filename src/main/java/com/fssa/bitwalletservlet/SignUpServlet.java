...
public class SignUpServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (userName == null || "".equals(userName)) {
			...
			response.sendRedirect("SignUp.jsp?errorMessage=Invalid UserName");
		}

		else if (email == null || "".equals(email)) {
			out.println("Invalid Email");
			response.sendRedirect("SignUp.jsp?errorMessage=Invalid Email");

		} else if (password == null || "".equals(password) || password.length() < 6) {
			response.sendRedirect("SignUp.jsp?errorMessage=Invalid Password");
		} else {
